...
<html lang="ja">
	<head>
		...
		<script type="text/javascript">
			...
			function check(){
				if(document.getElementsByName("consent_1[]")[0].checked && document.getElementsByName("consent_2[]")[0].checked && document.getElementsByName("consent_3[]")[0].checked){
					if( document.getElementsByName("consent_leader_name[]")[0].value!="" && document.getElementsByName("consent_tanto_name[]")[0].value!="" && document.getElementsByName("consent_tanto_mail[]")[0].value!=""){
					...
					alert("規約に同意いただけない場合は参加することができません。");
					return false;
				}
			}
	...
	<DIV class="main">
		...
		<H1>第30回七夕祭 同意フォーム</H1>
		<form action="post.php" method="post" enctype="multipart/form-data" name="consentform" onsubmit="return check()">
		<INPUT style="display: none;" name="formtype[]" value="form3">
		<div>
			<DIV class="bigsection">
				<BIGSECTION-TITLE>同意</BIGSECTION-TITLE>
				<P>以下をスクロールしてお読みのうえ、チェックボックスにチェックしてください。</P>
				<div class="rules">
					<div>
						<?php
							print( file_get_contents ("rules") );
						?>
					</div>
					<div class="concentform">
						<DIV><label><INPUT type="checkbox" name="consent_1[]" value="ok">第30回七夕祭参加規約を読み、それに同意する。</label></div>
						<DIV><label><INPUT type="checkbox" name="consent_2[]" value="ok">準備段階・当日では、実行委員会の指示に速やかに従うことに同意する。</label></div>
						<DIV><label><INPUT type="checkbox" name="consent_3[]" value="ok">参加の可否は、七夕祭実行委員会が決定権を有することに同意する。</label></div>
					</div>
				</div>
			</DIV>
			<DIV class="bigsection">
				<DIV class="smallsection">
					<SMALLSECTION-TITLE>団体名</SMALLSECTION-TITLE>
					<?php
						print('<INPUT name="groupname[]" value="'.$_COOKIE["groupname"].'">');
					?>
				</DIV>
				<DIV class="smallsection">
					<SMALLSECTION-TITLE>代表 <small style="color: #ff0000;">[必須]</small></SMALLSECTION-TITLE>
					<INPUT name="consent_leader_name[]">
				</DIV>
				<DIV class="smallsection">
					<SMALLSECTION-TITLE>担当者名 <small style="color: #ff0000;">[必須]</small></SMALLSECTION-TITLE>
					<INPUT name="consent_tanto_name[]">
				</DIV>
				<DIV class="smallsection">
					<SMALLSECTION-TITLE>担当者メールアドレス <small style="color: #ff0000;">[必須]</small></SMALLSECTION-TITLE>
					<INPUT name="consent_tanto_mail[]" type="email">
				</DIV>
			</DIV>
